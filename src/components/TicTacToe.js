...
const TicTacToe = () => {
  ...
  useEffect(() => {
    if (!getWinner(squares) && squares.every((item) => item !== "")) {
      ...
      setStatus(`Winner is ${getWinner(squares) === TicTacToeX ? "X" : "O"}. Please restart the game`);
    } else {
  ...
  return (
    <div className="tic-tac-toe-container">
      ...
      <h1>{status}</h1>
      <button onClick={handleRestart} className="restartButton"><img src={TicTacToeRestart} alt="" />Play Again</button>
    </div>
  );
}
